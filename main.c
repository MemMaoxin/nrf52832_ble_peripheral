...
#define DEVICE_NAME                     "kx4_15"                               /**< Name of device. Will be included in the advertising data. */
...
//					check=DataRead[0];
 ...
 //       }
        ...
        WriteToReg(0x58); //write to Communication register.The next step is to read from Data register.
				Delay(20);
	ReadFromReg(2, DataRead);
	//			NRF_LOG_INFO("DATAxxxxkkk:%x %x",DataRead[0],DataRead[1]);
/*	uint16_t radio=(DataRead[0]<<8) | DataRead[1];
	float R=radio*Rref/(65535-radio);
	float Ro=149;
	float T0=18;
	float  change_radio=0.5;
	float Tnow=T0+(R-Ro)/change_radio;
	uint8_t integer=(int)Tnow;
	uint8_t decimal=(int)((Tnow-integer)*100);
	uint8_t integer0=integer%10;
	uint8_t integer1=(integer%100-integer0)/10;
	DataRead[0]=integer1*16+integer0;
		uint8_t decimal0=decimal%10;
	uint8_t decimal1=(decimal%100-decimal0)/10;
	DataRead[1]=decimal1*16+decimal0;*/
		   err_code = ble_lbs_on_dht_change(&m_nus,DataRead[0],DataRead[1],m_conn_handle);
